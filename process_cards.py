...
def find_rectangles_with_morphology(gray_image, min_area, existing_rectangles, draw_debug=False, debug_dir=None, image_name=None):
    """
...
def find_rectangles_with_contour_merging(gray_image, edges, min_area, existing_rectangles, draw_debug=False, debug_dir=None, image_name=None):
    """
    ...
    if draw_debug and debug_dir is not None and image_name is not None:
        cv2.imwrite(os.path.join(debug_dir, f"{image_name}-adaptive.png"), adaptive_thresh)
        cv2.imwrite(os.path.join(debug_dir, f"{image_name}-adaptive-dilated.png"), dilated)
    ...
    if draw_debug and debug_dir is not None and image_name is not None:
        parts_image = np.zeros((gray_image.shape[0], gray_image.shape[1], 3), dtype=np.uint8)
        for part, _, _ in potential_parts:
            color = (0, 0, 255)  # Blue
            cv2.drawContours(parts_image, [part], 0, color, 2)
        cv2.imwrite(os.path.join(debug_dir, f"{image_name}-potential-parts.png"), parts_image)
    ...
    for region_idx, region in enumerate(card_regions):
        ...
        if draw_debug and debug_dir is not None and image_name is not None:
            cv2.imwrite(os.path.join(debug_dir, f"{image_name}-region-{region_idx}.png"), region_mask)
...
def crop_and_rotate_rectangles(
    ...
    if len(rectangles) < max_rectangles:
        ...
        extra_rectangles = find_rectangles_with_morphology(gray, min_area, rectangles, draw_contours, debug_dir if draw_contours else None, name)
        if extra_rectangles:
        ...
        if len(rectangles) < max_rectangles:
            ...
            merged_rectangles = find_rectangles_with_contour_merging(gray, edges, min_area, rectangles, draw_contours, debug_dir if draw_contours else None, name)
